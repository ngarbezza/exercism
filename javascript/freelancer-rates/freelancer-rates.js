...
export function priceWithMonthlyDiscount(ratePerHour, numDays, discount) {
  ...
  const priceForDiscountedMonths = dayRate(ratePerHour) * months * BILLABLE_DAYS_PER_MONTH * (1 - discount);
  const priceForRemainingDays = dayRate(ratePerHour) * remainingDays;
  return Math.ceil(priceForDiscountedMonths + priceForRemainingDays);
}
